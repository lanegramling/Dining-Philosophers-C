...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    pthread_mutex_lock(&waiter);    //LOCK WAITER & Wait until chopsticks are free
    while (!(left_chop_available(me) && right_chop_available(me))) pthread_cond_wait(&(me->can_eat), &waiter);
    *left_chop_available(me) = 0;  //Take chopsticks
    *right_chop_available(me) = 0;
    pthread_mutex_unlock(&waiter);  //UNLOCK WAITER
     ...
     */
    pthread_mutex_lock(&waiter);   //LOCK WAITER
    *left_chop_available(me) = 1;  //Return chopsticks
    *right_chop_available(me) = 1;
    pthread_cond_broadcast(&(right_phil(me)->can_eat)); //Broadcast relevant can_eat signals
